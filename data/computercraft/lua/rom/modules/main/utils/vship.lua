...
-- NO ROTATION ORDER
function vship.getVectorYaw()
    local look = ship.getLook()
    return math.atan2(look.z, look.x)
end

function vship.getVectorPitch()
    local look = ship.getLook()
    return math.asin(look.y)
end

function vship.getAngularVelocity()
    local o = ship.getOmega()
    local angVel = transformVector(ship.getQuaternion(), vector.new(o.x, o.y, o.z))
